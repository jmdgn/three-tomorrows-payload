...
export function useAnimationBottom() {
  ...
  useEffect(() => {
    ...
    eventListenersRef.current.push({ target: window, event: 'scroll', handler: debouncedHandleScroll });

    function animate() {
      if (!isMounted) return;
      
      if (window._isServicePanelVisible === true) {
        if (window._frameCount === undefined) window._frameCount = 0;
        window._frameCount++;
        
        if (window._frameCount % 2 !== 0) {
      ...
      if (window.sphere && window.water) {
        ...
        if (sp >= scrollStartThreshold && sp <= scrollEndThreshold) {
          const normalizedProgress = (sp - scrollStartThreshold) / 
        ...
        if (window.camera) {
          window.camera.position.y = THREE ? THREE.MathUtils.lerp(30, 5, sp * 1.2) : 30 - 25 * (sp * 1.2);
          window.camera.position.z = THREE ? THREE.MathUtils.lerp(100, 40, sp * 1.1) : 100 - 60 * (sp * 1.1);
        }
    ...
    const setupBubbleContainerScrollFade = () => {
      ...
      eventListenersRef.current.push({ target: window, event: 'scroll', handler: debouncedScrollDirectionHandler });
      ...
      const combinedObserver = new IntersectionObserver((entries) => {
        ...
        entries.forEach(entry => {
          ...
          if (targetId.includes('service-panel') && bubbleContainer) {
            ...
            } else if (!entry.isIntersecting && entry.boundingClientRect.top > 0 && !isFactoidsVisible) {
    ...
    const setupFactoidFrameScript = () => {
      ...
      stickyElem.style.transition = "width 0.5s cubic-bezier(0.23, 1, 0.32, 1), height 0.5s cubic-bezier(0.23, 1, 0.32, 1), border-radius 0.5s cubic-bezier(0.23, 1, 0.32, 1), top 0.5s cubic-bezier(0.23, 1, 0.32, 1)";
      
      stickyElem.style.width = "5vw";
      stickyElem.style.height = "5vh";
      stickyElem.style.borderRadius = "32px";
      ...
      function updateSizeAndOpacity() {
        if (!isMounted || !section) return;
        
        const rect = section.getBoundingClientRect();
        const windowHeight = window.innerHeight;
        
        const startEnter = windowHeight * 0.8;
        const endEnter = windowHeight * 0.2;
        let enterProgress = (startEnter - rect.top) / (startEnter - endEnter);
        enterProgress = Math.max(0, Math.min(1, enterProgress));
        
        const startExit = windowHeight * 0.8;
        const endExit = windowHeight * 0.2;
        let exitProgress = (startExit - rect.bottom) / (startExit - endExit);
        exitProgress = Math.max(0, Math.min(1, exitProgress));
        
        const totalProgress = enterProgress * (1 - exitProgress);
        
        const newWidth = 5 + (95 * totalProgress);
        const newHeight = 5 + (95 * totalProgress);
        const newBorderRadius = 32 - (32 * totalProgress);
        const newTop = 50 - (50 * totalProgress);
        
        const fadeTrigger = 20;
        const fadeDistance = 100;
        let opacityProgress = (fadeTrigger + fadeDistance - rect.top) / fadeDistance;
        opacityProgress = Math.max(0, Math.min(1, opacityProgress));
        
        if (Math.abs(parseFloat(cachedDimensions.width) - newWidth) > 0.1) {
          cachedDimensions.width = `${newWidth}vw`;
        }
        
        if (Math.abs(parseFloat(cachedDimensions.height) - newHeight) > 0.1) {
          cachedDimensions.height = `${newHeight}vh`;
        }
        
        if (Math.abs(parseFloat(cachedDimensions.borderRadius) - newBorderRadius) > 0.1) {
          cachedDimensions.borderRadius = `${newBorderRadius}px`;
        }
        
        if (Math.abs(parseFloat(cachedDimensions.top) - newTop) > 0.1) {
          cachedDimensions.top = `${newTop}px`;
        }
        
        if (Math.abs(parseFloat(cachedDimensions.opacity) - opacityProgress) > 0.01) {
          cachedDimensions.opacity = opacityProgress.toString();
        }
      }
      
      function applyChanges() {
        if (!isMounted || !stickyElem || !contentFull) return;
        
        stickyElem.style.width = cachedDimensions.width;
        stickyElem.style.height = cachedDimensions.height;
        stickyElem.style.borderRadius = cachedDimensions.borderRadius;
        stickyElem.style.top = cachedDimensions.top;
        contentFull.style.opacity = cachedDimensions.opacity;
      }
      ...
      eventListenersRef.current.push({ target: window, event: 'scroll', handler: handleScroll });
      eventListenersRef.current.push({ target: window, event: 'resize', handler: handleScroll });
    ...
    const setupFactoidCardsFade = () => {
        ...
        requestAnimationFrame(() => {
          ...
          elements.forEach(element => {
            ...
            if(element.classList.contains('factoidSingle-container')) {
              const opacity = 1 - Math.min(1, Math.max(0, 
                (factoidFadeStart - distanceFromTop) / 
                (factoidFadeStart - factoidFadeEnd)
              ));
            ...
            if (element.classList.contains('stickyContent-title') && parseFloat(element.style.opacity) > 0.1) {
              const letters = element.querySelectorAll('span');
              const totalLetters = letters.length;
              
              const colorProgress = Math.min(1, Math.max(0, 
                (titleThreshold * colorStartMultiplier - distanceFromTop) / titleThreshold * colorSpeed
              ));
      ...
      eventListenersRef.current.push({ target: window, event: 'scroll', handler: throttledUpdateOpacity });
      eventListenersRef.current.push({ target: window, event: 'resize', handler: throttledUpdateOpacity });
