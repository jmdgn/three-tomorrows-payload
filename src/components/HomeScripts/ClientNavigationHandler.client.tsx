...
const HomePageNavigationHandler = dynamic(
  () => import('./HomePageNavigationHandler.client'),
  { ssr: false }
)

export default function ClientNavigationHandler() {
  useEffect(() => {
    if (typeof window !== 'undefined' && typeof window.initThreeJS === 'function' && !window.threeJSInitialized) {
